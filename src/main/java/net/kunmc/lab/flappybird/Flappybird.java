    ...
    public void start() {
        ...
        new BukkitRunnable() {
            ...
            public void run() {
                ...
                if (count > 0) {
                    Bukkit.getOnlinePlayers().forEach(player -> {
                        player.sendTitle(String.valueOf(count), isClickMode() ? "スペースを２回押すか羽根を持ってクリックでジャンプ！" : "スペースを２回押してジャンプ！", 0, 25, 10);
                    ...
                    Bukkit.getOnlinePlayers().forEach(player -> {
                        player.sendTitle("スタート！", isClickMode() ? "スペースを２回押すか羽根を持ってクリックでジャンプ！" : "スペースを２回押してジャンプ！", 0, 25, 10);
